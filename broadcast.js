...
async function getCamera(deviceId, maxWidth, maxHeight) {
    ...
    try {
        // Let's try with ideal width and height constraints
...
async function handleVideoDeviceSelect() {
    ...
    //const cameraStream = await getCamera(deviceId);
...
const myIngestEndpointInputEl = document.getElementById("ingest-endpoint");
const myStreamKeyInputEl = document.getElementById("stream-key");
const mystreamKey = getParameterByName("key",window.location.href);
const endpoint = getParameterByName("endpoint",window.location.href);
if(mystreamKey){
    myStreamKeyInputEl.value = mystreamKey;
}
if(endpoint){
    myIngestEndpointInputEl.value = endpoint;
}
